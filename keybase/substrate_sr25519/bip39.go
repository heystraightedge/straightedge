...
func PrivKeyFromMnemonic(mnemonic string, password string) (sr25519.PrivKeySr25519, error) {
	seed, err := SeedFromMnemonic(mnemonic, password)
	if err != nil {
		return sr25519.PrivKeySr25519{}, err
	}
	var secret [32]byte
	copy(secret[:], seed[:32])
	return sr25519.PrivKeySr25519(secret), nil
}
...
func MiniSecretFromMnemonic(mnemonic string, password string) (*schnorrkel.MiniSecretKey, error) {
	...
	var secret [32]byte
	copy(secret[:], seed[:32])
	return schnorrkel.NewMiniSecretKeyFromRaw(secret)
}
