...
// CreateTestApp will create a new strd application and provide money to every address
// listed there
func CreateTestApp(t *testing.T, accounts []*auth.BaseAccount) *app.StraightedgeApp {
	fmt.Printf("%#v\n", accounts[1])
	genAccounts := make([]authExported.GenesisAccount, len(accounts))
	for i, acct := range accounts {
		genAccounts[i] = acct
	}
	strd := SetupWithGenesisAccounts(genAccounts)
	return strd
}
...
func TestSendWithApp(t *testing.T) {
	...
	straightedge := CreateTestApp(t, accts)
	...
	_ = sign(t, straightedge, msg, &keys[0], true)
}

func sign(t *testing.T, straightedge *app.StraightedgeApp, msg sdk.Msg, signer *signer, expectPass bool) *sdk.Result {
	_, res, _ := SignAndDeliver(t, straightedge, []sdk.Msg{msg}, []uint64{signer.acctNum}, []uint64{signer.seq}, expectPass, signer.priv)
	if expectPass {
		signer.seq++
	}
	return res
